...
function Check-ffplay {
    If (Test-Path -Path "$SkinsPath..\CoreData\IdleStyle\ffplay.exe") {
        $RmAPI.Bang('[!SetOption Button110 MeterStyle "Set.Button:S | AvaButtonYes"][!SetOption Value110 MeterStyle "Set.String:S | Set.Value:S | AvaTextYes"][!UpdateMeterGroup Ava][!Redraw]')
    } else {
        $RmAPI.Bang('[!SetOption Button110 MeterStyle "Set.Button:S | AvaButtonNo"][!SetOption Value110 MeterStyle "Set.String:S | Set.Value:S | AvaTextNo"][!UpdateMeterGroup Ava][!Redraw]')
    }
}

function Check-default {
    If ($RmAPI.VariableStr('Videoname') -match "DefaultVideo") {
        $RmAPI.Bang('[!WriteKeyvalue Variables VideoPath "'+$RmAPI.VariableStr('SKINSPATH')+$RmAPI.VariableStr('Skin.Name')+'\@Resources\Global\DefaultVideo.mp4" "'+$RmAPI.VariableStr('Sec.SaveLocation')+'"]')
    }
}
